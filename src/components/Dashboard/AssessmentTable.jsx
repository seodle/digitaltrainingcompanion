...
const AssessmentTable = ({ 
  ...
  const getAssessmentUserId = (a) => {
    ...
    return typeof a.userId === 'object' ? a.userId?._id : a.userId;
  };
  // Return populated owner object when available (used for displaying names)
  const getAssessmentOwner = (a) => (
    a?.userId && typeof a.userId === 'object' ? a.userId : null
  );
  ...
  const isOwner = (assessment) => {
    ...
    console.log("Current state:", {
        ...
        doesAssessmentHaveUserId: !!getAssessmentUserId(assessment),
        userId: getAssessmentUserId(assessment),
    ...
    const isOwned = getAssessmentUserId(assessment) === currentUser._id;
    console.log("Ownership check result:", isOwned);
    console.log("Assessment userId:", getAssessmentUserId(assessment));
 ...
 */
  ...
  const renderNumberFieldCell = (params, field) => {
    ...
    const assessmentUserId = getAssessmentUserId(assessment);
    const isAssessmentOwner = !assessmentUserId || assessmentUserId === currentUser._id;
    const isAssessmentOwnedByMonitoringOwner = assessmentUserId && (assessmentUserId === monitoring?.userId);
  ...
  /**
    ...
    */
    ...
    try {
        ...
        await axios.put(
            `${BACKEND_URL}/assessments/${assessmentId}`, 
            updatedAssessment, 
            {
                headers: {
...
/**
  ...
  if (ownerFilter === 'mine') {
        const assessmentUserId = getAssessmentUserId(assessment);
        return !assessmentUserId || assessmentUserId === currentUser._id; 
  }
  ...
  const handleDragEnd = async (result) => {
    ...
    try {
        const token = localStorage.getItem("token");
        await axios.put(
          `${BACKEND_URL}/assessments/monitoring/${currentMonitoringId}/resequence`,
