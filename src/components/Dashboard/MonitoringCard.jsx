...
const handleUpdateMonitoring = async (field, value) => {
  ...
  try {
    ...
    await axios.put(`${BACKEND_URL}/monitorings/${monitoring._id}`, updatedMonitoring, {
      headers: {
        Authorization: `Bearer ${token}`
      }
    });
 ...
 */
  const handleShareMonitoring = async (event) => {
    ...
    try {
        ...
        await axios.put(`${BACKEND_URL}/monitorings/${monitoring._id}/startSharing`, { sharingCode: code }, {
        headers: {
            Authorization: `Bearer ${token}`
        }
        });
...
const handleRemoveSharingCode = async (monitoringId) => {
  console.log(`Stop sharing/following for monitoring ID`, monitoringId);
  try {
    const token = localStorage.getItem("token");
    const isOwner = currentUser._id === monitoring.userId;
    const endpoint = isOwner
      ? `${BACKEND_URL}/monitorings/${monitoringId}/stopSharing`
      : `${BACKEND_URL}/users/monitorings/${monitoringId}/stopFollowing`;
    await axios.put(
      endpoint,
      {},
      {
    ...
    await loadMonitoringAndAssessments(currentUser, setMonitorings, setAssessments, setCurrentMonitoringId);
    if (!isOwner) {
      setUsersWhoRedeemed(prev =>
        prev.filter(u => String(u._id ?? u.id) !== String(currentUser._id ?? currentUser.id))
      );
    } else {
      setUsersWhoRedeemed([]);
    }
  } catch (err) {
    console.error('Error stopping sharing/following:', err);
  ...
  >
    <DialogTitle>
      {warningDialog.type === 'delete' 
        ...
        : getMessage(isOwner ? 'label_stop_sharing' : 'label_stop_following')}
    </DialogTitle>
    <DialogContent>
      <DialogContentText>
        {warningDialog.type === 'delete'
    ...
    >
      ...
      >
        ...
        >
            ...
            {(isOwner ? (monitoring.sharingCode && usersWhoRedeemed.length > 0) : monitoring.sharingCode) && (
              <Tooltip title={getMessage(isOwner ? "label_stop_sharing" : "label_stop_following")}>
              <IconButton
                  onClick={(e) => {
                  e.stopPropagation();
