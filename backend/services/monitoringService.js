...
/**
 * Gets all users who have redeemed a specific sharing code and the owner info.
 * 
 * @param {string} code - The sharing code to look up
 * @returns {Promise<Object>} { result: { usersRedeemed, owner }, status } or { error, status }
 */
const getUsersByRedeemedCode = async (code) => {
  try {
    ...
    const monitoring = await Monitoring.findOne({ sharingCode: code });
    ...
    const owner = await User.findById(monitoring.userId, 'firstName lastName -_id').lean();
...
/**
 * Stops sharing for a monitoring: removes the sharing code from all users and clears it on the monitoring.
 * @param {string} monitoringId
 * @returns {Promise<Object>} { message } or { error, status }
 */
const stopSharingMonitoring = async (monitoringId) => {
  try {
    ...
    // Remove code from all users who redeemed it
    await User.updateMany(
      { sharingCodeRedeemed: codeToRemove },
      { $pull: { sharingCodeRedeemed: codeToRemove } }
    );
    // Clear the sharing code on the monitoring
    await Monitoring.updateOne(
      { _id: monitoringId },
      { $set: { sharingCode: null } }
    );
    return { message: 'Sharing stopped successfully', status: 200 };
  } catch (error) {
    console.error('Error in stopSharingMonitoring:', error);
    return { error: 'Server error during operation', status: 500 };
  }
};

/**
 * Starts sharing for a monitoring by setting a provided sharing code.
 * @param {string} monitoringId
 * @param {string} sharingCode
 * @returns {Promise<Object>} { result } or { error, status }
 */
const startSharingMonitoring = async (monitoringId, sharingCode) => {
  try {
    if (!sharingCode || typeof sharingCode !== 'string' || !sharingCode.trim()) {
      return { error: 'Missing or invalid sharing code', status: 400 };
    }
    ...
    if (!updatedMonitoring) {
      return { error: 'No monitoring found with the given ID', status: 404 };
    }

    return { result: updatedMonitoring, status: 200 };
  } catch (error) {
    console.error('Error in startSharingMonitoring:', error);
    return { error: 'Server error during operation', status: 500 };
  }
};
/**
...
const createMonitoring = async (monitoringData) => {
  try {
    ...
    let newMonitoring = new Monitoring({
      ...
      courseAiBeaconId: monitoringData.courseAiBeaconId ?? null,
...
module.exports = {
  ...
  updateMonitoring, deleteMonitoring, stopSharingMonitoring, startSharingMonitoring, getUsersByRedeemedCode
};