...
/**
 * Fetches a specific assessment by its associated monitoring ID and creation date.
 * @param {string} monitoringId - The unique identifier of the monitoring document.
 * @param {Date} creationDate - The creation date of the assessment to be retrieved.
 * @return {Promise<Object>} A promise that resolves to the assessment document if found.
 * @throws {Error} Throws an error if the monitoring or assessment is not found, or if there is an error during the query execution.
 */
const getAssessmentByDateAndMonitoringId = async (monitoringId, creationDate) => {
    try {
        // Convert creation date to ISO format to match the database format
        const isoCreationDate = new Date(creationDate).toISOString();

        // Query to find the assessment using monitoringId and creationDate range
        ...
        if (!foundAssessment) {
            throw new Error("No assessment found with the given creation date");
        }

        return foundAssessment;
    } catch (error) {
        console.error(error);
        throw new Error("An error occurred while getting the assessment");
    }
};
...
/**
 * Fetches all assessments associated with a specific monitoring ID.
 * @param {string} monitoringId - The unique identifier of the monitoring whose assessments are to be retrieved.
 * @return {Promise<Array>} A promise that resolves to an array of assessment documents associated with the given monitoring ID.
 * @throws {Error} Throws an error if there's an issue executing the query.
 */
const getAssessmentsByMonitoringId = async (monitoringId) => {
    try {
        const assessments = await Assessment.find({ monitoringId: monitoringId });
        return assessments;
    } catch (error) {
        console.error(error);
...
/**
 * Retrieves all assessments of a specified type for a given monitoring ID and transforms them to match frontend format.
 * 
 * @param {string} assessmentType - The type of assessment to retrieve (e.g., 'TRAINEE_CHARACTERISTICS', 'LEARNING')
 * @param {string} monitoringId - The ID of the monitoring session
 * @returns {Promise<Array<{id: string, name: string}>>} Array of assessment objects with transformed IDs and names
 * @throws {Error} If database query fails or transformation encounters an error
 * 
 * @example
 * try {
 *   const assessments = await getAssessmentsByType('LEARNING', '507f1f77bcf86cd799439011');
 *   // Returns: [{id: "507f1f77bcf86cd799439011", name: "Learning Assessment 1"}, ...]
 * } catch (error) {
 *   console.error('Failed to get assessments:', error);
 * }
 */
const getAssessmentsByType = async (assessmentType, monitoringId) => {
    try {
        // Get all assessments of the specified type for the given monitoring ID
        const assessments = await Assessment.find(
            {
                type: assessmentType,
                monitoringId: monitoringId // Add monitoring ID to query
            },
            { _id: 1, name: 1 }
        ).lean();
        ...
        console.error('Error in getAssessmentsByType:', error);
        throw error;
    }
};

module.exports = {
    createAssessment, getAssessmentByDateAndMonitoringId, deleteAssessment, updateAssessmentSurvey, getAssessmentsByType,
    getAssessmentsByMonitoringId, updateAssessment, deleteAssessmentsFromMonitoring, fetchSurveyData, copyAssessmentsByMonitoringId,
